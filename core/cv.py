...
from core.helper import G, logwrap
from core.settings import Settings as ST  # noqa
from core.error import TargetNotFoundError, InvalidMatchingMethodError
from utils.transform import TargetPos

from aircv.template_matching import TemplateMatching
from aircv.keypoint_matching import KAZEMatching, BRISKMatching, AKAZEMatching, ORBMatching
from aircv.keypoint_matching_contrib import SIFTMatching, SURFMatching, BRIEFMatching
...
@logwrap
def loop_find(serialno, query, timeout=ST.FIND_TIMEOUT, threshold=None, interval=0.01, intervalfunc=None):
    """
    ...
    while True:
        screen = G.DEVICE[serialno].snapshot(filename=None)
        ...
        if screen is None:
            ...
            if match_pos:
                try_log_screen(serialno, screen)
                return match_pos
        ...
        # 不能一直停留在这里, 需要切换到别的协程去
        with suppress(Exception):
            gevent.sleep(0.01)


@logwrap
def try_log_screen(serialno, screen=None):
    """
    ...
    if not ST.LOG_DIR:
        return
    if screen is None:
        screen = G.DEVICE[serialno].snapshot()
