...
from core.cv import Template, loop_find, try_log_screen
from core.error import TargetNotFoundError
from core.settings import Settings as ST
from utils.compat import script_log_dir
from core.helper import (G, delay_after_operation, import_device_cls,
...
@logwrap
def snapshot(serialno, filename=None, msg=""):
    """
    ...
    if filename:
        ...
        screen = G.DEVICE[serialno].snapshot(filename)
        return try_log_screen(serialno, screen)
    else:
        return try_log_screen(serialno)
...
@logwrap
def touch(serialno, v, times=1, **kwargs):
    """
    ...
    if isinstance(v, Template):
        pos = loop_find(serialno, v, timeout=ST.FIND_TIMEOUT)
    else:
        try_log_screen(serialno)
        pos = v
    for _ in range(times):
        G.DEVICE[serialno].touch(pos, **kwargs)
        # gevent.sleep(0.01)
...
@logwrap
def text(serialno, text, enter=True, **kwargs):
    """
    ...
    :platforms: Android, Windows, iOS
    """
    G.DEVICE[serialno].text(text, enter=enter, **kwargs)
    delay_after_operation()
...
@logwrap
def wait(serialno, v, timeout=None, interval=0.5, intervalfunc=None):
    """
    ...
    pos = loop_find(serialno, v, timeout=timeout, interval=interval, intervalfunc=intervalfunc)
    return pos


@logwrap
def exists(serialno, v):
    """
    ...
    """
    try:
        pos = loop_find(serialno, v, timeout=ST.FIND_TIMEOUT_TMP)
