...
from aircv import imread, get_resolution
from aircv.utils import compress_image
from utils.compat import decode_path, script_dir_name
...
class LogToHtml(object):
    ...
    def __init__(self, serialno, script_root, log_root="", static_root="", export_dir=None, script_name="", logfile=LOGFILE, lang="en", plugins=None):
        self.log = []
    ...
    def _analyse(self, serialno):
        ...
        # pprint(steps)
        translated_steps = [self._translate_step(serialno, s) for s in steps]
        return translated_steps

    def _translate_step(self, serialno, step):
        ...
        screen = self._translate_screen(serialno, step, code)
    ...
    def _translate_screen(self, serialno, step, code):
        ...
        for item in step["__children__"]:
            if item["data"]["name"] == "try_log_screen" and isinstance(item["data"].get("ret", None), six.text_type):
                ...
                if self.export_dir:  # all relative path
                    screen['_filepath'] = "images\\%s" % src
                else:
                    screen['_filepath'] = "images\\%s" % src
                screen['src'] = "%s\\screenshots\\%s" % (serialno.replace(".", "").replace(":", ""), src)
    ...
    def _translate_code(self, step):
        ...
        for k, arg in enumerate(args):
            ...
            if isinstance(value, dict) and value.get("__class__") == "Template":
                ...
                arg["image"] = "images\\%s" % value["filename"]
    ...
    def report(self, serialno, template_name, output_file=None, record_list=None):
        self._load()
        steps = self._analyse(serialno)
